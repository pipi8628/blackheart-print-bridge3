...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        root.addView(tv("🏷️ 黑心純列印 V3.8", 28, Color.WHITE, true));
        ...
        Button startBtn = btn("▶️ 開始監聽", Color.rgb(6,214,160));
        Button stopBtn = btn("⏸ 停止監聽", Color.rgb(239,71,111));
        Button testBtn = btn("🧪 測試列印", Color.rgb(142,202,230));
    ...
    private TextView label(String text) { return tv(text, 18, Color.rgb(255,209,102), true); }
        ...
        new Thread(() -> {
            try {
                ...
                if (!job.optBoolean("ok", false)) {
                    ui(() -> { status("監聽中｜沒有待列印貼紙"); working = false; });
                    return;
                }
                ...
                ui(() -> { status("收到 #" + orderNo + " 第 " + labelNo + " 張，正在列印..."); log("EZPL:\n" + ezpl); });

                sendSocket(ezpl);
                httpGet(base + "?api=done&row=" + enc(row) + "&id=" + enc(id));

                printedCount++;
                ui(() -> { countText.setText("已列印：" + printedCount); status("列印完成 #" + orderNo + " 第 " + labelNo + " 張"); working = false; });

            } catch (Exception ex) {
                ui(() -> { status("錯誤：" + ex.getMessage()); log(ex.toString()); working = false; });
            }
        }).start();
    }
        ...
        new Thread(() -> {
            try {
                String ezpl = "^Q30,2\r\n" +
                        ...
                        "AA,20,72,1,1,0,0E,PURE PRINT\r\n" +
                        "AA,20,108,1,1,0,0E,NO ARCORE\r\n" +
                        "E\r\n";
                sendSocket(ezpl);
                ui(() -> status("測試列印已送出"));
            } catch (Exception ex) {
                ui(() -> { status("測試失敗：" + ex.getMessage()); log(ex.toString()); });
            }
        }).start();
    }
